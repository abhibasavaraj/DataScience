...
app.layout = html.Div(children=[
    ...
    html.H3('Comparision of different statistics', style = {'textAlign' : 'center'}),
    ...
    html.Div([dcc.Graph(id = 'confirmed_stats')]),
    html.Div([dcc.Graph(id = 'recovered_stats')]),
    html.Div([dcc.Graph(id = 'death_stats')])
])

@app.callback([Output(component_id='country_confirmed', component_property='figure'),
...
@app.callback([Output(component_id = 'confirmed_stats', component_property = 'figure'),
Output(component_id = 'recovered_stats', component_property = 'figure'),
Output(component_id = 'death_stats', component_property = 'figure')],
[Input(component_id = 'multi_countries', component_property = 'value')])

def countries_statistics(multi_countries):
    
    confirmed_figure = go.Figure()
    recovered_figure = go.Figure()
    deaths_figure = go.Figure()
    
    for each_country in multi_countries:
        dates = []
        confirmed_data = []
        recovered_data = []
        death_data = []

        response = requests.get(f"https://api.covid19api.com/total/country/{each_country}")
        ...
        confirmed_figure.add_traces( go.Scatter(x = dates, y = confirmed_data, mode='lines+markers', name = each_country))
        recovered_figure.add_traces( go.Scatter(x = dates, y = recovered_data, mode='lines+markers', name = each_country))
        deaths_figure.add_traces( go.Scatter(x = dates, y = death_data, mode='lines+markers', name = each_country))


    confirmed_figure.update_layout(
    height = 800, 
    title = {
        'text' : 'Confirmed cases comparision',
        'x' : 0.5,
    },
    xaxis_title = 'Date',
    ...
    return confirmed_figure, recovered_figure, deaths_figure
